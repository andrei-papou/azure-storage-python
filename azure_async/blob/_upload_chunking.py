...
from azure_async.common._common_conversion import _encode_base64
from azure_async.common._error import _ERROR_VALUE_SHOULD_BE_SEEKABLE_STREAM
...
class _PageBlobChunkUploader(_BlobChunkUploader):
    async def _upload_chunk(self, chunk_start, chunk_data):
        chunk_end = chunk_start + len(chunk_data) - 1
        resp = await self.blob_service._update_page(
            self.container_name,
            self.blob_name,
            chunk_data,
            chunk_start,
