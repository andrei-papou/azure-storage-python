...
class ListGenerator(AsyncIterable):
    """
    A generator object used to list azure_async resources. The generator will lazily
...
class RetryContext(object):
    ...
    :ivar ~azure.azure_async.common._http.HTTPRequest request:
        The request sent to the azure_async service.
    :ivar ~azure.azure_async.common._http.HTTPResponse response:
        The response returned by the azure_async service.
...
class RetentionPolicy(object):
    ...
    is independent of the total limit for your azure_async account.
    ...
    def __init__(self, enabled=False, days=None):
        ...
        :param bool enabled: 
            ...
            azure_async service. If disabled, logging and metrics data will be retained
...
class Logging(object):
    ...
    to a azure_async service. This information can be used to monitor individual requests
    and to diagnose issues with a azure_async service. Requests are logged on a best-effort
    basis.

    All logs are stored in block blobs in a container named $logs, which is
    automatically created when Storage Analytics is enabled for a azure_async account.
    The $logs container is located in the blob namespace of the azure_async account.
...
class Metrics(object):
    ...
    to a azure_async service. Transactions are reported at both the API operation level
    as well as at the azure_async service level, and capacity is reported at the azure_async
    service level. Metrics data can be used to analyze azure_async service usage, diagnose
    issues with requests made against the azure_async service, and to improve the
...
class ServiceProperties(object):
    ...
    azure_async service, including Analytics and CORS rules.

    Azure Storage Analytics performs logging and provides metrics data for a azure_async
    account. You can use this data to trace requests, analyze usage trends, and 
    diagnose issues with your azure_async account. To use Storage Analytics, you must
    ...
    http://azure.microsoft.com/documentation/articles/azure_async-monitoring-diagnosing-troubleshooting/
...
class ServiceStats(object):
    ...
    geo-redundant replication is enabled for the azure_async account.
