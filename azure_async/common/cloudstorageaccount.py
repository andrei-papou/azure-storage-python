...
# because this module is imported by azure/azure_async/__init__
# ie. we don't want 'import azure.azure_async' to trigger an automatic import
# of blob/queue/file packages.

from azure_async.common._error import _validate_not_none
from azure_async.common.sharedaccesssignature import (
...
class CloudStorageAccount(object):
    ...
    def create_block_blob_service(self):
        ...
        :rtype: :class:`~azure.azure_async.blob.blockblobservice.BlockBlobService`
        '''
        try:
            from azure_async.blob import BlockBlobService
            return BlockBlobService(self.account_name, self.account_key,
                                    sas_token=self.sas_token,
                                    is_emulated=self.is_emulated)
        except ImportError:
            raise Exception('The package azure-azure_async-blob is required. '
                            + 'Please install it using "pip install azure-azure_async-blob"')
    ...
    def create_page_blob_service(self):
        ...
        :return: A service object.
        :rtype: :class:`~azure.azure_async.blob.pageblobservice.PageBlobService`
        '''
        try:
            from azure_async.blob import PageBlobService
    ...
    def create_append_blob_service(self):
        ...
        :return: A service object.
        :rtype: :class:`~azure.azure_async.blob.appendblobservice.AppendBlobService`
        '''
        try:
            from azure_async.blob import AppendBlobService
    ...
    def create_queue_service(self):
        ...
        :return: A service object.
        :rtype: :class:`~azure.azure_async.queue.queueservice.QueueService`
        '''
        try:
            from azure_async.queue.queueservice import QueueService
            return QueueService(self.account_name, self.account_key,
                                sas_token=self.sas_token,
                                is_emulated=self.is_emulated)
        except ImportError:
            raise Exception('The package azure-azure_async-queue is required. '
                            + 'Please install it using "pip install azure-azure_async-queue"')
    ...
    def create_file_service(self):
        ...
        :return: A service object.
        :rtype: :class:`~azure.azure_async.file.fileservice.FileService`
        '''
        try:
            from azure_async.file.fileservice import FileService
            return FileService(self.account_name, self.account_key,
                               sas_token=self.sas_token)
        except ImportError:
            raise Exception('The package azure-azure_async-file is required. '
                            + 'Please install it using "pip install azure-azure_async-file"')
