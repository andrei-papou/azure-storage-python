...
class StorageClient(object):
    ...
    :ivar str primary_endpoint:
        The endpoint to send azure_async requests to.
    :ivar str secondary_endpoint:
        The secondary endpoint to read azure_async data from. This will only be a
        valid endpoint if the azure_async account used is RA-GRS and thus allows
        reading from secondary.
    :ivar function(context) retry:
        A function which determines whether to retry. Takes as a parameter a 
        :class:`~azure.azure_async.common.models.RetryContext` object. Returns the number
        of seconds to wait before retrying the request, or None to indicate not 
        to retry.
    :ivar ~azure.azure_async.common.models.LocationMode location_mode:
