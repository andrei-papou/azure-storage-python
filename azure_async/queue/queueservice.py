...
from azure_async.common._http import (
    HTTPRequest,
)
...
class QueueService(StorageClient):
    ...
    :ivar function(data) encode_function: 
        ...
        :class:`~azure.azure_async.queue.models.QueueMessageFormat` for xml, base64 and
    ...
    :ivar function(data) decode_function: 
        ...
        See the :class:`~azure.azure_async.queue.models.QueueMessageFormat` for xml, base64
    ...
    async def get_queue_service_stats(self, timeout=None):
        '''
    ...
    async def get_queue_service_properties(self, timeout=None):
        '''
        Gets the properties of a azure_async account's Queue service, including
    ...
    async def set_queue_service_properties(self, logging=None, hour_metrics=None,
                                     minute_metrics=None, cors=None, timeout=None):
        '''
        Sets the properties of a azure_async account's Queue service, including
    ...
    async def list_queues(self, prefix=None, num_results=None, include_metadata=False,
        ...
        resp = await self._list_queues(**kwargs)

        return ListGenerator(resp, self._list_queues, (), kwargs)

    async def _list_queues(self, prefix=None, marker=None, max_results=None,
        ...
        return await self._perform_request(request, _convert_xml_to_queues, operation_context=_context)
    ...
    async def exists(self, queue_name, timeout=None):
        '''
        ...
        '''
        try:
            await self.get_queue_metadata(queue_name, timeout=timeout)
            return True
        except AzureHttpError as ex:
            _dont_fail_not_exist(ex)
            return False
    ...
    def _set_basic_headers(self, request):
        request.headers = {
            'Accept': '*/*',
            'Accept-Encoding': 'gzip, deflate',
            'Host': f'{self.account_name}.queue.core.windows.net',
